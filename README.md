...
Use this package in two passes.

### Pass A. Build the developmental backbone
Focus on:
- stage order
- timing windows
- explicit academic factors
- reproducible marker checkpoints

### Pass B. Add manufacturability bias
Use commercial/public signals to refine:
- anchor protocol
- stage handoff design
- feeder-free or scalable preferences
- harvest-state choice

But keep DOE conservative:
- test only explicit, controllable, interpretable variables first

## Current limitations

- Commercial web-search influence is intentionally conservative.
- Public company information is often high-level and not factor-complete.
- The package therefore separates `commercial influence` from `academic extraction` instead of mixing them blindly.
- For some target cell types, the commercial layer is much stronger than for others.

## Good next extension

If you want, the next upgrade should be:
- a real web-search backend module that stores timestamped source snippets
- source-tier scoring: `academic primary`, `commercial public`, `review`, `product page`
- explicit `anchor-only`, `DOE-eligible`, `exclude-from-DOE` labels for every factor
